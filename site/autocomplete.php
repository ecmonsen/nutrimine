...
$queryString = "select Long_Desc, match(Long_Desc) against (?) as Relevance "
        . "from FoodDescription fd "
        . "having Relevance > 0 "
        . "order by length(Long_Desc), Relevance desc "
        . "limit 25;";

//$queryString = "SELECT Long_Desc FROM FoodDescription "
//        . "where Long_Desc like ? "
//        . "order by length(Long_Desc) asc limit 25";
...
if ($stmt = $mysqli->prepare($queryString)) {
  ...
  if ($stmt->execute()) {
    ...
    $stmt->bind_result($longdesc, $relevance);
